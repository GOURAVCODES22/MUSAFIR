...
const COUNTRIES = [
  ...
  { name:"Patnitop", lat:33.0903, lon:75.3264, tz:"Asia/Kolkata", population:"~600 (hamlet)", area:"Udhampur district",
    ...
    photo:"https://commons.wikimedia.org/wiki/Special:FilePath/Img._Patnitop.jpg?width=1400",
    blurb:"A meadow-covered hill station at 2,024m with pine forests, viewpoints and nearby attractions including Nathatop and Sanasar.",
    places:[
      { name:"Nathatop", lat:33.105, lon:75.345, photo:"https://commons.wikimedia.org/wiki/Special:FilePath/Nathatop%2C_Jammu.jpg?width=1000" },
      { name:"Sanasar Lake", lat:33.1268, lon:75.2598, photo:"https://commons.wikimedia.org/wiki/Special:FilePath/Sanasar_Lake_Jammu.jpg?width=1000" },
      { name:"Patnitop Park", lat:33.0910, lon:75.3260, photo:"https://i0.wp.com/travelshoebum.com/wp-content/uploads/2018/03/dsc_7289.jpg?resize=1200%2C795&ssl=1" },
      { name:"Naag Temple", lat:33.0918, lon:75.3269, photo:"https://media1.thrillophilia.com/filestore/dw1auyy3l911g6r9b8pmglv8metj_1-naag-mandir-patnitop-jammu-_-k.jpg" }
    ] },
  ...
  { name:"Kinnaur Kailash", lat:31.53, lon:78.25, tz:"Asia/Kolkata", population:"~500 (Kalpa village)", area:"Kinnaur district",
    ...
    photo:"https://commons.wikimedia.org/wiki/Special:FilePath/Shivling_%28monolithic_pillar%29.jpg?width=1400",
    blurb:"The sacred Kinnaur Kailash Shivling is a towering monolithic rock pillar revered on the Kinnaur Kailash pilgrimage." },
  { name:"Shrikhand Mahadev", lat:31.8464, lon:77.7396, tz:"Asia/Kolkata", population:"Uninhabited (pilgrimage trek)", area:"Kullu district",
    ...
    blurb:"Shrikhand Mahadev is known for its towering natural Shivling on the high Himalayan pilgrimage route in Himachal Pradesh." },
...
const JAMMU_KASHMIR_WEATHER_PLACES = [
  ['Jammu',32.7266,74.8570],['Kathua',32.3860,75.5180],['Samba',32.5742,75.1190],
  ['Udhampur',32.9160,75.1419],['Reasi',33.0810,74.8360],['Rajouri',33.3798,74.3152],
  ['Poonch',33.7703,74.0925],['Doda',33.1490,75.5484],['Ramban',33.2433,75.2430],
  ['Kishtwar',33.3137,75.7673],['Srinagar',34.0837,74.7973],['Anantnag',33.7311,75.1487],
  ['Kulgam',33.6446,75.0199],['Pulwama',33.8741,74.8996],['Shopian',33.7172,74.8341],
  ['Budgam',34.0159,74.6330],['Ganderbal',34.2260,74.7717],['Bandipora',34.4170,74.6410],
  ['Baramulla',34.2090,74.3428],['Kupwara',34.5262,74.2552]
].map(([name,lat,lon])=>({
  name:`${name}, Jammu and Kashmir`, lat, lon, tz:'Asia/Kolkata', photo:'', dynamic:true,
  blurb:`Live weather for ${name}, Jammu and Kashmir.`
}));

const weatherDirectory = new Map(JAMMU_KASHMIR_WEATHER_PLACES.map(p=>[p.name.toLowerCase().split(',')[0],p]));
JAMMU_KASHMIR_WEATHER_PLACES.forEach(p=>weatherDirectory.set(p.name.toLowerCase().split(',')[0],p));

async function loadWorldWeatherDirectory(){
  try{
    const res = await fetch('https://restcountries.com/v3.1/all?fields=name,capital,latlng');
    if(!res.ok) return;
    ...
    list.forEach(c=>{
      ...
      const place={name,lat:Number(coords[0]),lon:Number(coords[1]),tz:'',photo:'',dynamic:true,
        blurb:`Live weather lookup for ${name}.`};
      if(Number.isFinite(place.lat)&&Number.isFinite(place.lon) && !weatherDirectory.has(name.toLowerCase())) weatherDirectory.set(name.toLowerCase(),place);
      const cap=Array.isArray(c.capital)&&c.capital[0] ? c.capital[0] : null;
      if(cap && !weatherDirectory.has(cap.toLowerCase())) weatherDirectory.set(cap.toLowerCase(),{...place,name:`${cap}, ${name}`});
    });
    const dl=document.getElementById('countryList');
    if(dl){
      const existing=new Set(Array.from(dl.options).map(o=>o.value.toLowerCase()));
      weatherDirectory.forEach(p=>{ if(!existing.has(p.name.toLowerCase())){ const o=document.createElement('option'); o.value=p.name; dl.appendChild(o); } });
    }
  }catch(e){}
}
...
async function geocodeWeatherPlace(query){
  try{
    ...
    return {
      ...
      photo:'', dynamic:true,
      blurb:`Live weather lookup for ${[r.name, r.admin1, r.country].filter(Boolean).join(', ')}.`
    };
  }catch(e){ return null; }
}
...
async function doWeatherSearch(){
  ...
  const district = weatherDirectory.get(val.replace(/,.*$/,'').trim()) || weatherDirectory.get(val);
  if(district){ msg.textContent=''; loadWeatherSingle(district); return; }
...
loadWeatherSingle(weatherDirectory.get('jammu') || COUNTRIES.find(c=>c.name==="Jammu") || COUNTRIES[0]);
loadWorldWeatherDirectory();
...
function openPanel(country){
  panel.classList.toggle('is-local-place', !!country.isLocalPlace);
  ...
  if(country.isLocalPlace){
    panelImgEl.style.objectFit = 'contain';
    panelImgEl.style.objectPosition = 'center center';
    panelImgEl.style.background = '#000';
    panelImgEl.style.padding = '0';
  } else if(country.dynamic){
    panelImgEl.style.objectFit = 'contain';
    panelImgEl.style.objectPosition = 'center center';
