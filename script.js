...
// The globe must never depend on a remote image to boot. Start with a visible
// ocean-blue planet immediately, then upgrade it to the Earth texture if the
// network responds. This prevents the mobile loader from getting stuck at 0%.
...
loadingManager.onProgress = (url, loaded, total) => {
  ...
  if(txt) txt.textContent = 'LOADING THE WORLD — '+pct+'%';
};
loadingManager.onLoad = () => finishLoader();
...
const earthTex = texLoader.load(
  ...
  undefined,
  () => {
    // Primary source failed — try backup, but never keep the UI blocked.
    texLoader.load(
      EARTH_TEX_BACKUP,
      (tex) => {
        globeMat.map = tex;
...
// Hard timeout: slow/blocked image CDNs must not leave Musafir frozen on 0%.
...
function setRotateMode(on){
  ...
  if(badge) badge.textContent = rotateMode ? '✋ Double-tap/click to scroll' : '👆 Double-tap or double-click to rotate';
}
setRotateMode(false);
