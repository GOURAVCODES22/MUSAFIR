...
// starfield
{
  ...
  const starMat = new THREE.PointsMaterial({ color:0xffffff, size:0.35, transparent:true, opacity:0.65 });
...
const LABEL_PLACES = [
  ...
  { country:"Italy", label:"Rome" },
  { country:"Japan", label:"Tokyo" },
  { country:"Indonesia", label:"Bali" },
  { country:"South Africa", label:"Cape Town" },
];
const placeTagsEl = document.getElementById('placeTags');
const placeTagEls = LABEL_PLACES.map(p => {
  const c = COUNTRIES.find(x => x.name === p.country);
  const el = document.createElement('div');
  el.className = 'place-tag';
  el.innerHTML = `<span class="pin">📍</span><span>${p.label}</span>`;
  if(placeTagsEl && c) placeTagsEl.appendChild(el);
  return c ? { country:c, el } : null;
}).filter(Boolean);
...
function updatePlaceTags(){
  ...
  placeTagEls.forEach(({country, el}) => {
    ...
    if(facing < 0.15){ el.classList.remove('visible'); return; }
...
function openPanel(country){
  ...
  gsap.set(panel, { scale:1.04 });
  panel.classList.add('open');
  gsap.to(panel, { scale:1, duration:0.7, ease:"power3.out" });
}
function closePanel(){
...
- Kuldeep Singh lives in Parladhpur and owns Shreeganesh Glass House. He is a professional in glass cutting and aluminium work, handling all types of modern glass/aluminium fabrication.
