...
function setRotateMode(on){
  ...
  canvas.style.touchAction = rotateMode ? 'none' : 'pan-y';
...
canvas.addEventListener('pointerup', (e)=>{
  ...
  if(!moved && tapDt<TAP_MAX_DURATION && rotateMode && !flying){
...
const placeTagEls = COUNTRIES.map(c => {
  ...
  // Labels are visual only. Let the canvas own touch/pointer gestures so a drag
  // across a country label cannot accidentally trigger a country selection.
...
(function initStatsCountUp(){
  const statEls = Array.from(document.querySelectorAll('.stats .stat-num'));
  ...
  const run = () => {
    ...
    const tick = (now) => {
      ...
      targets.forEach(({el, value, suffix, decimals}) => {
