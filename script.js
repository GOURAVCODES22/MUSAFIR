...
function setRotateMode(on){
  ...
  if(badge){
    badge.textContent = rotateMode
      ...
      : '👆 Double-tap to explore the globe';
  }
}
setRotateMode(false);
...
canvas.addEventListener('pointerdown', (e)=>{
  // A double tap/click unlocks the globe controls and smoothly zooms in.
  const now = performance.now();
  if(now-lastTapTime < 340 && e.pointerType !== 'mouse'){
    setRotateMode(!rotateMode);
    smoothZoom(rotateMode ? camDist-1.0 : camDist+1.0);
  }
  if(e.pointerType === 'mouse' && now-lastTapTime < 340){
    setRotateMode(!rotateMode);
  }
  lastTapTime = now;
  if(rotateMode) startDrag(e);
}, {passive:true});
...
// Pinch zoom works whenever globe controls are unlocked.
canvas.addEventListener('pointerdown', (e)=>{
  if(!rotateMode || e.pointerType!=='touch') return;
...
// Badge also acts as a reliable touch control.
document.getElementById('rotateBadge').addEventListener('click',(e)=>{
  e.preventDefault();
  setRotateMode(!rotateMode);
  if(rotateMode) smoothZoom(camDist-0.7);
});
...
function closePanel(){
  ...
  // Back to Home must return to the actual home/hero section, not merely
  // hide the country panel while leaving the visitor halfway down the page.
  // Use a smooth scroll when supported, with an immediate fallback.
