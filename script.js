...
/* =========================================================
   GLOBE INTERACTION — NORMAL AUTO-ROTATE / DOUBLE-TAP EXPLORE
========================================================= */
let dragging=false, lastX=0, lastY=0, velX=0, velY=0, downX=0, downY=0, downTime=0, moved=false;
let autoRotate=true, flying=false, rotateMode=false;
let lastTapTime=0, lastTapX=0, lastTapY=0;
let pinchStartDist=null, pinchStartCamDist=camDist;
let activePointerId=null, suppressTap=false;
...
function smoothZoom(targetDist){
  ...
  gsap.to(camera.position,{z:target,duration:0.55,ease:'power2.out',onUpdate:()=>{camDist=camera.position.z;}});
}

function setRotateMode(on){
  rotateMode=!!on;
  canvas.style.touchAction=rotateMode?'none':'manipulation';
  canvas.style.cursor=rotateMode?'grab':'';
  const badge=document.getElementById('rotateBadge');
  if(badge) badge.textContent=rotateMode ? '✨ Explore mode • Drag • Pinch • Tap a country' : '👆 Double-tap to explore';
  if(!rotateMode){
    dragging=false; activePointerId=null; pinchStartDist=null; velX=velY=0;
    autoRotate=true;
  }
}
setRotateMode(false);
...
function toggleExplore(){
  if(flying) return;
  if(rotateMode){ setRotateMode(false); }
  else { setRotateMode(true); autoRotate=false; }
}

function startDrag(e){
  if(!rotateMode || flying) return false;
...
canvas.addEventListener('pointerdown',(e)=>{
  if(!rotateMode) return; // Normal mode: no globe interaction; page scrolling remains native.
  startDrag(e);
},{passive:true});

canvas.addEventListener('pointermove',(e)=>{
  if(!rotateMode || !dragging || activePointerId!==e.pointerId || flying) return;
  const dx=e.clientX-lastX, dy=e.clientY-lastY;
  if(Math.abs(e.clientX-downX)>4 || Math.abs(e.clientY-downY)>4) moved=true;
  applyDelta(dx,dy); velX=dx; velY=dy; lastX=e.clientX; lastY=e.clientY;
  e.preventDefault();
},{passive:false});

canvas.addEventListener('pointerup',(e)=>{
  let wasTap=false;
  if(rotateMode && activePointerId===e.pointerId){
    const tapDt=performance.now()-downTime;
    wasTap=!moved && tapDt<320;
    dragging=false; activePointerId=null;
    try{canvas.releasePointerCapture(e.pointerId);}catch(_){ }
    if(Math.abs(velX)>0.4 || Math.abs(velY)>0.4){
      const vx=velX,vy=velY;
      gsap.to({}, {duration:0.55,ease:'power2.out',onUpdate:function(){applyDelta(vx*(1-this.progress())*0.8,vy*(1-this.progress())*0.8);}});
    }
  }

  // Touch double-tap is handled on pointerup so ordinary page scrolling is never hijacked.
  if(e.pointerType==='touch' || e.pointerType==='pen'){
    const now=performance.now();
    const dx=e.clientX-lastTapX,dy=e.clientY-lastTapY;
    const isDoubleTap=now-lastTapTime<360 && Math.hypot(dx,dy)<28;
    if(isDoubleTap){
      suppressTap=true;
      toggleExplore();
      lastTapTime=0;
    } else if(wasTap || !rotateMode){
      lastTapTime=now; lastTapX=e.clientX; lastTapY=e.clientY;
    }
  }

  if(rotateMode && wasTap && !suppressTap && !flying){
    const hit=raycastMarker(e.clientX,e.clientY);
    if(hit) flyTo(hit.userData.country);
  }
  suppressTap=false;
  if(rotateMode && !dragging) setTimeout(()=>{if(rotateMode&&!dragging&&!flying) autoRotate=false;},2200);
},{passive:true});

canvas.addEventListener('pointercancel',()=>{
  dragging=false; activePointerId=null; pinchStartDist=null;
});
...
canvas.addEventListener('touchstart',(e)=>{
  ...
  pinchStartDist=Math.hypot(dx,dy); pinchStartCamDist=camDist; autoRotate=false;
},{passive:true});
...
canvas.addEventListener('touchmove',(e)=>{
  ...
  camDist=Math.min(11,Math.max(3.25,pinchStartCamDist*(pinchStartDist/dist)));
...
if(rotateBadge) rotateBadge.addEventListener('dblclick',(e)=>{e.preventDefault();toggleExplore();});
...
function findPlaceSearch(raw){
  ...
  const country = COUNTRIES.find(c => c.name === 'Jammu');
  return country && (country.places || []).find(p => normalizeSearch(p.name).includes('shreeganeshglasshouse')) || null;
}
...
async function doSearch(){
  ...
  if(placeMatch){
    ...
    flyTo({
      ...placeMatch,
      population: '—', area: 'Kot Bhalwal, Jammu', tz: 'Asia/Kolkata',
      winter: '—', summer: '—', dynamic: false, isLocalPlace: true
    });
    return;
  }
