...
COUNTRIES.forEach(c=>{
  ...
  // glow ring sprite behind marker
  const ringMat = new THREE.SpriteMaterial({ color:0xd4af6a, transparent:true, opacity:0.35, depthWrite:false });
  const ring = new THREE.Sprite(ringMat);
  ring.scale.set(0.13,0.13,1);
...
document.addEventListener('click', (e)=>{ if(e.target.closest('#panelClose')) closePanel(); });
document.addEventListener('touchend', (e)=>{ if(e.target.closest('#panelClose')) closePanel(); });
