...
const COUNTRIES = [
  ...
  { name:"Jammu", lat:32.7266, lon:74.8570, tz:"Asia/Kolkata", population:"1.5 million (district)", area:"3,097 km²",
    ...
    places:[SHREE_GANESH_GLASS_HOUSE]
  },
...
// Draw a local procedural Earth texture first so the globe is visible even when
// an external texture CDN is slow/blocked. A remote texture may replace it later.
function makeEarthFallbackTexture(){
  const c=document.createElement('canvas');
  c.width=1024; c.height=512;
  const ctx=c.getContext('2d');
  const g=ctx.createLinearGradient(0,0,0,c.height);
  g.addColorStop(0,'#061a35'); g.addColorStop(.45,'#0b3d69'); g.addColorStop(1,'#031326');
  ctx.fillStyle=g; ctx.fillRect(0,0,c.width,c.height);
  // stylised landmasses — deliberately soft so the globe still reads as cinematic.
  const land=[
    [[90,115],[150,78],[205,96],[245,145],[222,190],[180,174],[150,210],[112,180]],
    [[245,210],[300,225],[320,285],[292,350],[255,322],[240,275]],
    [[430,105],[490,75],[560,92],[600,125],[585,170],[535,184],[505,155],[450,165],[410,140]],
    [[565,180],[635,190],[690,230],[675,280],[625,300],[590,260]],
    [[690,250],[760,230],[825,270],[850,325],[815,365],[750,350],[710,310]],
    [[775,105],[830,85],[885,125],[875,170],[830,188],[795,155]],
    [[870,355],[925,345],[955,380],[930,420],[885,410]]
  ];
  ctx.fillStyle='#2f7659';
  land.forEach(poly=>{ctx.beginPath(); poly.forEach((p,i)=>i?ctx.lineTo(p[0],p[1]):ctx.moveTo(p[0],p[1])); ctx.closePath(); ctx.fill();});
  // night-side city-light specks
  ctx.fillStyle='rgba(255,210,120,.75)';
  for(let i=0;i<260;i++){
    const x=Math.random()*c.width, y=80+Math.random()*330;
    ctx.fillRect(x,y,Math.random()<.85?1:2,Math.random()<.85?1:2);
  }
  return new THREE.CanvasTexture(c);
}

const globeMat = new THREE.MeshPhongMaterial({ map:makeEarthFallbackTexture(), color:0xffffff, shininess:10, specular:0x294b73 });

// Upgrade to the real Earth image when available, but never make the page wait for it.
try{
  const texLoader = new THREE.TextureLoader();
  texLoader.setCrossOrigin('anonymous');
  texLoader.load(
    'https://raw.githubusercontent.com/mrdoob/three.js/r128/examples/textures/planets/earth_atmos_2048.jpg',
    (tex)=>{ tex.anisotropy=renderer.capabilities.getMaxAnisotropy(); tex.minFilter=THREE.LinearMipmapLinearFilter; tex.magFilter=THREE.LinearFilter; globeMat.map=tex; globeMat.needsUpdate=true; },
    undefined,
    ()=>{}
  );
}catch(e){}
...
// The interactive scene is ready; don't wait on remote images.
setTimeout(()=>{ const l=document.getElementById('loader'); if(l) l.classList.add('hidden'); }, 350);
...
let rotateMode = false; // default: page scrolls smoothly like the rest of the site. Double-tap / double-click enables rotate + pinch/wheel zoom.
let lastTapTime = 0;
...
function setRotateMode(on){
  ...
  if(badge) badge.textContent = rotateMode ? '✋ Double-tap/click to rotate' : '👆 Double-tap or double-click to rotate';
}
setRotateMode(false);

let tapDownX=0, tapDownY=0, tapDownTime=0, tapMoved=false;
canvas.addEventListener('pointerdown', (e)=>{
  tapDownX = e.clientX; tapDownY = e.clientY; tapDownTime = performance.now(); tapMoved = false;
  if(!rotateMode) return; // let the page scroll/behave completely normally
  dragging = true; moved = false; autoRotate = false;
  lastX = downX = e.clientX; lastY = downY = e.clientY; downTime = performance.now();
  velX = velY = 0;
  canvas.setPointerCapture(e.pointerId);
});
canvas.addEventListener('pointermove', (e)=>{
  if(Math.abs(e.clientX-tapDownX)>6 || Math.abs(e.clientY-tapDownY)>6) tapMoved = true;
  const hit = raycastMarker(e.clientX, e.clientY);
  document.getElementById('cursor-ring')?.classList.toggle('hover', !!hit);
  canvas.style.cursor = (rotateMode && hit) ? 'pointer' : (rotateMode ? 'grab' : '');
...
window.addEventListener('pointerup', (e)=>{
  // tapping directly on a marker always works, whether rotate-mode is on or off
  const tapDt = performance.now()-tapDownTime;
  if(!tapMoved && tapDt < 400 && !flying){
    const hit = raycastMarker(e.clientX, e.clientY);
...
// double-click (mouse) / double-tap (touch) toggles rotate mode on/off
canvas.addEventListener('dblclick', ()=> setRotateMode(!rotateMode));
canvas.addEventListener('touchend', (e)=>{
  if(e.touches.length > 0) return; // ignore while a second finger is still down (pinch)
  const now = Date.now();
  if(now - lastTapTime < 320){ setRotateMode(!rotateMode); dragging = false; }
  lastTapTime = now;
});

// two-finger pinch to zoom — only active once rotate mode is on
...
canvas.addEventListener('touchmove', (e)=>{
  ...
  if(e.touches.length === 2 && pinchStartDist){
    ...
    camDist = Math.min(11, Math.max(3.2, pinchStartCamDist * (pinchStartDist/dist)));
    camera.position.z = camDist;
  }
}, { passive:false });
canvas.addEventListener('touchend', (e)=>{
  if(e.touches.length < 2){ pinchStartDist = null; setTimeout(()=>{ autoRotate = true; }, 1800); }
});

canvas.addEventListener('wheel', (e)=>{
  if(!rotateMode) return; // let the page scroll normally with the mouse wheel
  e.preventDefault();
  camDist = Math.min(11, Math.max(3.6, camDist + e.deltaY*0.004));
  camera.position.z = camDist;
}, { passive:false });
...
const placeTagEls = COUNTRIES.map(c => {
  ...
  el.addEventListener('click', ()=>{ if(!flying) flyTo(c); });
  el.addEventListener('touchend', (e)=>{ e.preventDefault(); e.stopPropagation(); if(!flying) flyTo(c); });
...
function renderCountryMap(country){
  ...
  locations.forEach((loc, i)=>{
    ...
    m.bindPopup(`
      <div style="text-align:center;min-width:150px">
        <img src="${loc.photo}" style="width:100%;height:90px;object-fit:cover;border-radius:8px;margin-bottom:6px" onerror="this.style.display='none'">
        <b style="font-size:0.85rem">${loc.name}</b><br>
        <a href="${loc.directionsUrl || `https://www.google.com/maps/dir/?api=1&destination=${loc.lat},${loc.lon}`}" target="_blank" rel="noopener noreferrer" style="color:#d4af6a;font-size:0.78rem">Directions →</a>
      </div>
    `);
    markers.push(m);
  });
  ...
  document.getElementById('panelDirections').href =
    `https://www.google.com/maps/dir/?api=1&destination=${country.lat},${country.lon}`;
...
function openPanel(country){
  ...
  favBtn.classList.toggle('active', favSet.has(country.name));
  favBtn.onclick = ()=>{
    if(favSet.has(country.name)) favSet.delete(country.name); else favSet.add(country.name);
...
window.__closePanel = closePanel; // exposed globally so the inline onclick="" on the button always works, no matter what
const panelCloseBtn = document.getElementById('panelClose');
panelCloseBtn.addEventListener('click', closePanel);
panelCloseBtn.addEventListener('touchend', (e)=>{ e.preventDefault(); closePanel(); });
panelCloseBtn.addEventListener('touchstart', (e)=>{ e.stopPropagation(); });
// ultra-safe fallback: catch any tap/click that lands inside the close button, even if the direct
// listener above somehow gets missed (e.g. a stray overlay briefly intercepting the event)
document.addEventListener('click', (e)=>{ if(e.target.closest('#panelClose')) closePanel(); }, true);
document.addEventListener('touchend', (e)=>{ if(e.target.closest('#panelClose')) closePanel(); }, true);
...
const SHREE_GANESH_GLASS_HOUSE = {
  ...
  lat:32.8120, lon:74.8188,
  tz:"Asia/Kolkata",
  population:"—",
  area:"Kot Bhalwal, Jammu",
  winter:"—", summer:"—",
...
function isShreeGaneshSearch(value){
  const key = value.toLowerCase().replace(/[^a-z0-9]/g,'');
  return key.includes('shreeganeshglasshouse') || key === 'ganeshglasshouse' ||
         key === 'shreeganesh' || key.includes('shreeganeshglasshousejammu');
}
...
async function doSearch(){
  ...
  if(isShreeGaneshSearch(raw)){
    document.getElementById('searchMsg').textContent = '';
    document.getElementById('searchInput').value = '';
    flyTo(SHREE_GANESH_GLASS_HOUSE);
    return;
  }
...
const chipNames = ["India","Japan","France","UAE","Greece","Maldives","Jammu","Vaishno Devi"];
