...
(function(){
  ...
  const VISIBLE_COUNT = 6; // keep the page short: show this many, rest go in the "show more" box
  const moreWrap=$('rvMoreWrap'), moreList=$('rvMoreList'), moreBtn=$('rvMoreBtn');
  function render(items){
    ...
    sum.textContent = items.length
      ? (items.reduce((a,r)=>a+r.rating,0)/items.length).toFixed(1)+' ★ · '+items.length+' traveler review'+(items.length>1?'s':'')
      : 'Be the first to review Musafir';

    if(moreWrap){
      if(rest.length){
        moreWrap.hidden = false;
        moreList.replaceChildren(...rest.map(card));
        moreList.hidden = true;
        moreBtn.setAttribute('aria-expanded','false');
        moreBtn.textContent = 'Show '+rest.length+' more review'+(rest.length>1?'s':'');
        moreBtn.onclick = ()=>{
          const opening = moreList.hidden;
          moreList.hidden = !opening;
          moreBtn.setAttribute('aria-expanded', String(opening));
          moreBtn.textContent = opening ? 'Show less' : 'Show '+rest.length+' more review'+(rest.length>1?'s':'');
        };
      } else {
  ...
  async function load(){
    try{ const r=await fetch('/api/reviews'); render(r.ok ? ((await r.json()).reviews||[]) : []); }
    catch(_){ render([]); }
  }
